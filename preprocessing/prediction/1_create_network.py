...
def create_graph(df):
	...
	df_new = pd.read_csv('../domainsMissing.csv', header=None, index_col=0)
	newnodes = list(map(str,list(df_new.index)))
...
np.save('arrays/prediction/network/dist_mat_4_5.npy', dist_mat)
np.save('arrays/prediction/network/labels_100.npy', labels)

dist_df_orig.to_pickle('arrays/prediction/network/dist_df_orig_4_5.pkl')
