...
coord_in = Input(shape=(2*coord_df.shape[1],), dtype='int32', name='coord_in')
coord_net = Dense(128, activation=act)(coord_in)

dom_in = Input(shape=(domains_df.shape[1] * 2,), name='dom_in')
dom_net = Dense(128, activation=act)(dom_in)

x = keras.layers.concatenate([coord_in, dom_in])
x = Dense(128, activation=act)(x)

main_output = Dense(1, activation='sigmoid', name='output')(x)

model = Model(inputs=[coord_in, dom_in], outputs=main_output)
